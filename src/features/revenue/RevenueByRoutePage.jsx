...
export default function RevenueByRoute() {
  ...
  return (
    <div>
      <PageHeader title="Revenue by Route" subtitle="Invoiced revenue per route (= technician serving the customer) for the selected period (Remaining vs annual expected shows on All time). Click a route to drill into its customers." />
        ...
        {() => (
          <div className="space-y-5">
            ...
            <BarChartCard title={isAllTime ? 'Invoiced vs remaining by route' : 'Invoiced by route'} data={rows} xKey="routeCode"
              bars={isAllTime
                ? [{ key: 'invoiced', label: 'Invoiced', color: '#10B981', stackId: 'r' }, { key: 'remaining', label: 'Remaining', color: '#F59E0B', stackId: 'r' }]
